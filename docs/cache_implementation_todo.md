...
- [x] 4k. Implement `bptr_cache_release`: assert refcount>=2, decrement, if becomes 1 append to eviction queue; add `pool_idx_of` helper
- [x] 4l. Implement `bptr_cache_alloc`: find slot/evict, zero node, refcount=2, ht_insert; same eviction+flush logic as fetch
